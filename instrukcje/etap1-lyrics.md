...
**Docelowa długość utworu: 5–6 minut (obowiązuje od 2026-07-31).** Celem nie jest liczba wersów, ale **czas trwania gotowego audio: 5:00–6:00**. Przelicznik z pomiaru całego kanału: Suno śpiewa nasze teksty w tempie **7–10 sekund na śpiewaną linijkę** (mediana ok. 8,2 s), więc na 5–6 minut trzeba **ok. 46–54 linijek śpiewanych**, co przy standardowych powtórkach refrenu daje **ok. 30–40 unikalnych wersów**. Zakres obowiązujący dla nowych tekstów: **ok. 30–40 unikalnych wersów / 46–54 linijki śpiewane**.

Zastępuje wcześniejszą regułę „Psalm 27 (30 unikalnych wersów) jako górny limit" (obowiązywała od 2026-07-07). Powód zmiany: przy limicie 24–30 unikalnych wersów utwory wychodziły na 3:30–4:30, a Psalm 121 przy 28 unikalnych wersach wypadł na **3:04** — za krótko dla kanału. Dotyczy każdego nowego `lirycs.txt` od tej pory — nie dotyczy retroaktywnie utworów już ukończonych ani tych, gdzie audio zostało już wygenerowane w Suno (zmiana tekstu unieważniłaby dopasowanie audio i dalszych etapów).

**Długość weryfikuje się na gotowym audio, nie na liczbie wersów.** Tempo Suno waha się między generacjami w granicach **5,75–9,9 s na linijkę** (skrajne przypadki kanału: Psalm 121 — 32 linijki w 3:04; Psalm 90 — 34 linijki w 5:35), więc ten sam tekst może wyjść i na 4, i na 6 minut. Praktyka: wygenerować 2–3 podejścia i wybrać ujęcie mieszczące się w oknie 5:00–6:00 (przy równej frazie wokalnej — patrz „Praktyka generowania"). Jeśli wszystkie podejścia wychodzą krótsze, kolejność ratunkowa jest jak w pkt 10: najpierw dodatkowa powtórka refrenu w tekście, potem funkcja **Extend** w Suno.

**Rozbudowa przy psalmach krótkich.** Przy psalmach o małej liczbie wersetów (np. Psalm 121 — 8 wersetów) trafienie w 30–40 unikalnych wersów wymaga mocnego rozwinięcia poetyckiego, a nie dopisywania treści: każda nowa linijka musi dać się wskazać jako rozwinięcie konkretnego wersetu psalmu (pkt 2). Dopuszczalne narzędzia: dodatkowe `[Verse 3]`/`[Verse 4]`, powtórka `[Pre-Chorus]`, trzeci i czwarty `[Chorus]`, Outro domknięte zdublowaną frazą kluczową refrenu. Powtórki refrenu i Pre-Chorusa **muszą być identyczne słowo w słowo i tag w tag** — inaczej Suno je przearanżuje.
...
3. **Ma się ładnie rymować.** Piszemy tak, żeby wyszła ładna piosenka: rymy parami (AABB) lub przeplatane (ABAB), dopuszczalne rymy niedokładne/asonanse, jeśli brzmią naturalnie. Unikamy rymów częstochowskich na siłę — lepszy dobry asonans niż wymuszony rym. **Rytm jest ważniejszy niż idealny rym**: jeśli trzeba wybierać między dokładnym rymem a naturalnym akcentem frazy, wygrywa rytm.
4. **Równy rytm fraz i śpiewalność.** Wersy o zbliżonej liczbie sylab (ok. 10–14), krótkie frazy-oddechy (jedna fraza = jeden oddech). Długie, nierówne linijki powodują, że Suno „rozciąga" tekst i robi 2–3-sekundowe pauzy w środku wersu. Unikać trudnych do wyśpiewania zbitek spółgłosek (w polszczyźnie łatwo o „w brzmieniu chrzęstu" — takie sekwencje czytać na głos przed oddaniem tekstu; co się źle mówi, będzie się źle śpiewać i Suno częściej zniekształca tam wymowę).
4a. **Prosty, zrozumiały język — bez słów książkowych.** Tekst ma być zrozumiały przy pierwszym słuchaniu, więc unikamy słów uczonych, archaicznych i rzadkich zapożyczeń — np. „firmament", „ostoja", „kres", zwrot „stawać w zawody" (wszystkie wyłapane przy Psalmie 19) — na rzecz codziennych odpowiedników („niebo/nad ziemią", „niezawodny", „kraniec", „ruszać w zawody"). Test: jeśli słowo nie padłoby w zwykłej rozmowie ani w prostej piosence, poszukać prostszego synonimu, który utrzyma rym i rytm. Wyjątki — nie upraszczamy na siłę: utrwalone słownictwo biblijno-liturgiczne, które odbiorcy kanału dobrze znają („bojaźń", „Skała", „Zbawienie"), oraz powszechnie zrozumiałe słowa baśniowe („komnata"). Spłaszczenie tonu psalmu byłoby gorsze niż pojedyncze podniosłe słowo.

4b. **Poprawność gramatyczna form — sprawdzić przed oddaniem tekstu.** Śpiewany tekst bezlitośnie obnaża błędy fleksyjne, a Suno śpiewa dokładnie to, co dostanie. Przed zapisaniem `lirycs.txt` przejść tekst linijka po linijce pod kątem: poprawnych form imiesłowów („jest słyszany", nie „słyszony"), poprawnej rekcji czasowników („udzielić blasku" — dopełniacz, nie „udzielić blask"; „obmyć z pychy", ale „zachować od pychy") oraz zgodności rodzaju i liczby podmiotu z orzeczeniem i przydawkami. Wszystkie trzy typy wyłapane w Psalmie 19. W razie wątpliwości sprawdzić formę w słowniku zamiast zgadywać; jeśli poprawna forma psuje rym, przebudować cały wers albo zakończenie linijki partnerskiej — nigdy nie zostawiać błędnej formy dla ratowania rymu.
...
7a. **Najpierw refren (hook-first).** Refren decyduje o tym, czy pieśń zostaje w głowie, dlatego pracę nad tekstem zaczynamy od niego, nie od `[Verse 1]`: wybrać emocjonalne serce psalmu (pkt 8), napisać **3–5 wariantów refrenu**, ocenić je (czy fraza kluczowa zapada w pamięć po jednym przeczytaniu na głos? czy śpiewa się naturalnie? czy niesie centralną emocję psalmu?), wybrać najlepszy — i dopiero wokół niego budować zwrotki. Dobry refren zawiera **frazę kluczową powtórzoną lub sparalelizowaną** (np. anaforę „czemu... czemu...", refren Psalmu 42) — prostą, uniwersalną, wierną psalmowi. Wariantów nie zapisujemy w `lirycs.txt` (tam idzie tylko wersja finalna); można je pokazać użytkownikowi, jeśli wybór jest nieoczywisty. Narracji zwrotek nie projektujemy od zera — łuk (wprowadzenie → rozwinięcie/zmaganie → zwrot/kulminacja w `[Bridge]` → domknięcie w `[Outro]`) wyznacza sam psalm, bo zachowujemy jego obrazy i kolejność (pkt 2); zadbać jedynie, by podział na sekcje ten naturalny łuk respektował, a każda zwrotka prowadziła do refrenu i emocjonalnie go uzasadniała.
