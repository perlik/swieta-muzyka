...
1A. **Etap 1A — Master audio.** Przerobienie surowego pliku z Suno (`audio/audio.wav`) na `audio/audio_eq_v2.wav`: EQ (ubytek mułu w 300 Hz, obecność w 3 kHz, powietrze w 11.5 kHz), de-esser pasmowy na sybilanty („sz" 3–5 kHz, „ś" 4–9 kHz) i normalizacja do −14 LUFS / true peak −1.0 dBFS. EQ jest wpieczony w plik, więc po podmianie ścieżki **wyłączyć Equalizer w Resolve**. Uruchamia `instrukcje/skrypty/master_audio.py`. Długość pliku nie zmienia się co do próbki, więc nie wpływa to na napisy ani na oś czasu. **Model Claude: Haiku, effort low** (całość robi skrypt; jedyna ocena — czy sybilanty są dość ściszone — należy do użytkownika i jest odsłuchowa). Pełne zasady, uzasadnienie każdej wartości i lista pomysłów do przetestowania: `instrukcje/etap1a-audio-master.md`.
...
3. **Etap 3 — Prompty do obrazków.** Rozpisanie 16–26 kadrów (dosłowność, 10–20 s/kadr, lead ~1 s, łuk kolorystyczny, nić przewodnia) na bazie dokładnych znaczników czasowych z `txt/napisy.srt` (Etap 2). **Kadr 1 to zawsze kadr psalmistki** (młoda kobieta ok. 20 lat, twarz po prawej stronie kadru, wzrok wprost w kamerę) i **on jest zawsze bazą miniatury** dla Etapu 7 — pozycja stała, bez wyboru, ale zapisana jawnie w `prompty.md`. Pełna specyfikacja wyglądu i gotowy prompt: `etap3-prompty-stylu.md` § „Postacie". Wynik: `prompty.md` w podfolderze `prompts/`. **Warunek wstępny: musi już istnieć plik audio w podfolderze `audio/` ORAZ gotowy `txt/napisy.srt`** — jeśli któregoś z nich brakuje, nie rozpoczynać tego etapu, tylko zatrzymać się i zgłosić użytkownikowi brak. Wykonywać zawsze modelem **Fable** (jeśli bieżąca sesja nie działa na tym modelu, delegować do subagenta z `model: "fable"`). Pełne zasady stylu wizualnego, kanoniczny blok promptu, negative prompt i tempo montażu: `instrukcje/etap3-prompty-stylu.md`.
...
7. **Etap 7 — Miniatura.** Wygenerowanie miniatury 1280×720 na bazie kadru wskazanego w Etapie 3 (dla filmów od 2026-08-04 to zawsze kadr 1 — psalmistka; w starszych filmach inny kadr, wskazany w ich `prompty.md`; nie pytamy użytkownika o wybór), z napisem „Psalm X" + „śpiewany" na ciemnym panelu z równym paddingiem. **Model Claude: Haiku, effort low** (wygląd realizuje skrypt Pillow). Pełne zasady i nazewnictwo plików publikacyjnych: `instrukcje/etap7-miniatura.md`.
